    ...
    implements WindowListener, MouseListener, ActionListener
{
	...
	public Game(int i, MoveWriter movewriter)
    {
        ...
        myTurn = true;
        sendMove = null;
        System.err.println("Game: StrategoMini");
        if(Stratego.isVerbose())
            Stratego.traceIn("Game", "constructor", "playerNumber = " + i + "  sendMove = " + movewriter);
        playerNumber = i;
        sendMove = movewriter;
        opponentNumber = i != 1 ? 1 : 2;
        battleField = new Board(10,6 , i, this);
        tray = new Board(6, 4, i, this);
        tray.fillWithTokens();
        if(Stratego.isNetworked())
        {
            opponentTray = new Board(6, 4, opponentNumber);
            opponentTray.tokensVisible(false);
        } else
        {
            opponentTray = new Board(6, 4, opponentNumber, this);
        }
        ...
        ((AbstractButton) next).addActionListener(this);
        
        Insets insets = getInsets();
        Dimension dimension = control.getPreferredSize();
        int j = 14 * Stratego.getWidth() + insets.left + insets.right + insets.left;
    ...
    private void placeLake()
    {
       ...
       //battleField.placeToken(new Token(12, 0), 4, 3);
        ...
        battleField.placeToken(new Token(12, 0), 5, 4);
        battleField.placeToken(new Token(12, 0), 4, 4);
        battleField.placeToken(new Token(12, 0), 4, 1);
        battleField.placeToken(new Token(12, 0), 5, 1);
    ...
    private void stopGame()
    {
        ...
        int i = JOptionPane.showConfirmDialog(this, "Τέλος Παιχνιδιού;", "Επιβεβαίωση Εξόδου", 0, 3);
        if(i == 0)
        {
            if(Stratego.isNetworked())
    ...
    public void actionPerformed(ActionEvent actionevent)
    {
        ...
        if(obj == play)
        {
            if(!tray.isEmpty())
            {
                int i = JOptionPane.showConfirmDialog(this, "Το ταμπλό είναι άδειο, Θέλετε να ξεκινήσετε έτσι;", "Configuration Warning", 0);
                if(i == 1)
                    return;
            }
            checkReadiness();
        ...
        if(obj1 == next)
        {
            if(!tray.isEmpty())
            {
                int i = JOptionPane.showConfirmDialog(this, "Επόμενος Παίκτης.", "Σφάλμα", 0);
                
    	
                		
                						
                if(i == 1)return;   String tokenName[] = {
        "Flaaaaaaaaaag", "Marshal", "General", "Colonel", "Major", "Captain", "Lieutenant", "Sergeant", "Miner", "Scout", 
        "Spy", "Bomb", ""
    }; ;
